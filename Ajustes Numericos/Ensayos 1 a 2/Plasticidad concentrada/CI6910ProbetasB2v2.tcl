...
for {set i 1} {$i <= $nele} {incr i} {
	...
	element beamWithHinges $i $i $j 4 [expr 0.9*$colDepth] 4 [expr 0.9*$colDepth] $Ec $A $I $transfTag
...
recorder Node -file ProbetasB2disp.out -time -closeOnWrite -node $nodocentral -dof 2 disp
recorder Node -file ProbetasB2reac.out -time -closeOnWrite -node 1 [expr 1+$nele/15] [expr $nnodos-$nele/15] $nnodos -dof 2 reaction
