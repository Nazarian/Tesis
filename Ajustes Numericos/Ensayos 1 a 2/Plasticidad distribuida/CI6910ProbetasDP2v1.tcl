...
uniaxialMaterial Elastic 22 [expr 0.9*0.4*$Ec*$Acv]
...
for {set i 1} {$i <= $nele} {incr i} {
	...
	#element beamWithHinges $i $i $j 4 [expr 0.4*$colDepth] 4 [expr 0.4*$colDepth] $Ec $A $I $transfTag
...
recorder Node -file ProbetasDP2disp2.out -time -closeOnWrite -node $nodocentral -dof 2 disp
recorder Node -file ProbetasDP2reac2.out -time -closeOnWrite -node 1 [expr 1+$nele/15] [expr $nnodos-$nele/15] $nnodos -dof 2 reaction
