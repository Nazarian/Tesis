...
section Fiber 4 {
	...
	#layer straight 5 4 $As [expr $cover-$y1] [expr $z1-$cover] [expr $cover-$y1] [expr $cover-$z1]
	# barras del 22
	layer straight 9 2 $As2 [expr $cover+10-$y1] [expr -$cover] [expr $cover+30-$y1] [expr +$cover]  
	#barra del 28
...
for {set i 1} {$i <= $nele} {incr i} {
	...
	#element beamWithHinges $i $i $j 4 [expr 1.1*$colDepth] 4 [expr 1.1*$colDepth] $Ec $A $I $transfTag
...
recorder Node -file ProbetasCA2disp2.out -time -closeOnWrite -node $nodocentral -dof 2 disp
recorder Node -file ProbetasCA2reac2.out -time -closeOnWrite -node 1 [expr 1+$nele/15] [expr $nnodos-$nele/15] $nnodos -dof 2 reaction
...
#system SparseGEN
#system UmfPack 
integrator DisplacementControl  $nodocentral  2 -0.05
#integrator DisplacementControl 5 3 -0.0001; # displacement control algorithm seking constant increment of 0.1 at node 1 at 2'nd dof.
#analysis Transient 
analysis Static
