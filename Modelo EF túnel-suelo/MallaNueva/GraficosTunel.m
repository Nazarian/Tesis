...
w=0.25;
datoscualquiera=2*sin(w*(0:pi/10:(length(NodosLining)-1)*pi/10))+1;

datos=obtenerdatos(NodosLining);

datoscualquiera=-datos(:,1)/1000;

FDescala=10*2/max(abs(datoscualquiera));
%FDescala=0.05;
quiebre1=15;
quiebre2=155;
quiebrederecho=169;
...
for i=1:(length(indices1)-1)
    if (indices1(i)<quiebrederecho && indices1(i+1)<quiebrederecho)
        ...
        for j=indices1(i)+1:indices1(i+1)+1
...
plot([NodosLining(1,2) datosaplotear2(indices1(i+1)-quiebrederecho+2,1)],[NodosLining(1,3)-19 datosaplotear2(indices1(i+1)-quiebrederecho+2,2)],'Color',colores(rem(i,2)+1,:))
axis equal
