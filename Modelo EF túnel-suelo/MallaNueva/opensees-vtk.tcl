...
dict set __ele_nn zeroLength 2
dict set __ele_nn zeroLengthContact2D 2
dict set __ele_nn dispBeamColumn 2
dict set __ele_nn quad 4
dict set __ele_nn tri31 3
dict set __ele_nn 9_4_QuadUP 9
dict set __ele_nn elasticBeamColumn 2
dict set __ele_nn twoNodeLink 2
dict set __ele_nn truss 2


# init element type to vtk cell type dictionary
dict set __ele_vtk_cell zeroLength 3
dict set __ele_vtk_cell zeroLengthContact2D 3
dict set __ele_vtk_cell dispBeamColumn 3
dict set __ele_vtk_cell elasticBeamColumn 3
dict set __ele_vtk_cell quad 9
dict set __ele_vtk_cell tri31 5
dict set __ele_vtk_cell 9_4_QuadUP 28
dict set __ele_vtk_cell twoNodeLink 3
dict set __ele_vtk_cell truss 3


set renamed_model_command "__model__[clock clicks -microseconds]"
...
proc wrap_element_cmd {} {
  ...
  set renamed_element_command "__element__[clock clicks -microseconds]"
...
proc vtk_output_points {chan} {
  ...
  set nelems [getEleTags]
  set num_of_nodes [llength $nids]
  set num_of_elements [llength $nelems]
...
proc vtk_output_cell_data_header {chan} {
  set nelems [getEleTags]
  set num_of_elements [llength $nelems]
  puts $chan ""
  puts $chan "CELL_DATA $num_of_elements"
}
...
proc vtk_output_ele_resp_stresses {chan} {
  set nelems [getEleTags]
  set num_of_elements [llength $nelems]
  set cur_time [getTime]
  set ndf [get_ndf]

  puts $chan "FIELD ele_resp_$cur_time 1"
  puts $chan "stresses_$cur_time 22 $num_of_elements float"
  foreach n $nelems {
    set resp [string trim [eleResponse $n stress]]
    set resp [regexp -inline -all {\S+} $resp]
    while {[llength $resp] < 3} { lappend resp 0.0 }
    if {$ndf > 3} { set resp [lrange $resp 0 2] }
    puts $chan $resp
  }
  puts $chan ""
}

proc vtk_output_ele_resp_strains {chan} {
  set nelems [getEleTags]
  set num_of_elements [llength $nelems]
  set cur_time [getTime]
  set ndf [get_ndf]

  puts $chan "FIELDS ele_resp_$cur_time float"
  foreach n $nelems {
    set resp [string trim [eleResponse $n strain]]
    set resp [regexp -inline -all {\S+} $resp]
    while {[llength $resp] < 3} { lappend resp 0.0 }
    if {$ndf > 3} { set resp [lrange $resp 0 2] }
    puts $chan $resp
